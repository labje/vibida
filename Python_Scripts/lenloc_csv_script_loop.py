...
def getXBRLData(xbrlFile, csvFile, dsFile, op):
	...
	con = [] #contendra el nombre las posibles columnas
	...
	for row in reader: #Se va recorriendo el fichero CSV mapa
		lista = []; #lista de contextos para la cuenta de la row del CSV mapa correspondiente
		...
		# contextos contiene la lista de columnas formateada para la cuenta de la row del CSV mapa correspondiente
		contextos = ["{" + NS[row["Lenloc_context"+ str(n)].split(":")[0]]+"}" + row["Lenloc_context"+ str(n)].split(":")[1] for n in range(numContextos)]

		cantidad = 0
		if(len(lista) > 0): #hay entrada para la cuenta
...
# python lenloc_csv_script_loop.py lista_fichs_xbrl.txt mapas_csv\lenloc3-econ-ingr-cuentas-label.csv name-domain\name-domain-lenloc3.json 1:N
# python lenloc_csv_script_loop.py lista_fichs_xbrl.txt mapas_csv\lenloc3-econ-gast-cuentas-label.csv name-domain\name-domain-lenloc3.json 1:N
# python lenloc_csv_script_loop.py lista_fichs_xbrl.txt mapas_csv\lenloc3-prog-cuentas-label.csv name-domain\name-domain-lenloc3.json 1:N